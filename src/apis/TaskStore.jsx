...
const TaskStore = (props) => {
    ...
    const handleCreate = (e) => {
        ...
        if (e.target[0].value.length !== 0)
            setData([...data, { task: e.target[0].value, id: Math.random() * 1000000000, timestamp: ` ${time[0]}:${time[1]}:${time[2]}  ` }])
        e.target[0].value = "";
    }
    ...
    return (
        <Tasks.Provider value={{ data, handleCreate, handleDelete, handleEdit, setData, edit }}>
            {props.children}
        </Tasks.Provider>
    )
}
